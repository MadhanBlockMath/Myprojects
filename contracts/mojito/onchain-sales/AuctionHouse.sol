...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard, ERC1155TokenReceiver {
    ...
    // a mapping for the quantity of nft. 
    mapping(uint256 => uint256) nftQuantity;

    bytes4 constant erc20_interfaceId = 0x80ac58cd; // 721 interface id
    ...
    constructor(address _nftAddress, address _weth) {
        require(
            (IERC165(_nftAddress).supportsInterface(erc20_interfaceId) ||
    ...
    function createAuction(
        ...
        require(
            (IERC165(tokenContract).supportsInterface(erc20_interfaceId) ||
        ...
        nftQuantity[auctionId] = quantity;
        if (IERC165(tokenContract).supportsInterface(erc20_interfaceId)) {
            require (
                nftQuantity[auctionId] == 1,
                "quantity for 721 should be equal to one"
                );

            tokenOwner = IERC721(tokenContract).ownerOf(auctions[auctionId].tokenId);
            require(
                msg.sender == IERC721(tokenContract).getApproved(auctions[auctionId].tokenId) ||
            ...
            require(
                nftQuantity[auctionId] <= ownerBalance && nftQuantity[auctionId] != 0,
        ...
        auctionExists(auctionId)
    {
        ...
        require(
            ...
            "reservePrice should be lesser than maxPrice"
        );
    ...
    function _cancelAuction(uint256 auctionId) internal {
        ...
        uint256 quantity = nftQuantity[auctionId] ;
        if (
