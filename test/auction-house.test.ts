...
describe("Auction Contract", () => {
  ...
  beforeEach(async () => {
    ...
    nftMedia = await NFTMedia.deploy(
      "Sero NFT",
      "SFT",
      BASE_URI,
      BASE_URI_SUFFIX,
  ...
  describe("Create Auction House", () => {
    ...
    it("should revert if the token ID does not exist", async () => {
      ...
      ).to.be.rejectedWith(Error, "ERC721: owner query for nonexistent token");
    });
    ...
    it("should revert if the reservePrice is greater than maxPrice", async () =>{
      ...
      expect(await nftMedia.tokenURI(0)).to.equal(
        `${BASE_URI}0${BASE_URI_SUFFIX}`
      );
      await expect (auctionHouse
        .connect(addr1)
        .createAuction(
          tokenId,
          nftMedia.address,
    ...
    it("should be automatically approved if the creator is the Zero Address", async () => {
      const [addr1, expectedCurator] = await ethers.getSigners();
      await nftMedia
        .connect(addr1)
        .setApprovalForAll(auctionHouse.address, true);
      await nftMedia.mintTo(addr1.address, BID_SHARES);
  ...
  describe("Set Auction Approval", () => {
    ...
    beforeEach(async () => {
      ...
      const tokenId = 0;
      let duration = 60 * 60 * 24;
      const reservePrice = BigNumber.from(10).pow(18).div(4);
      const maxPrice = BigNumber.from(10).pow(18);
      const startTime = 1655904918;
      const quantity = 1;
    ...
    it("should revert if the auction has already started", async () => {
      ...
      await auctionHouse
        .connect(bidder)
        .createBid(0, ONE_ETH.div(2), { value: ONE_ETH.div(2) });
      await expect(
  ...
  describe("Create Bid", () => {
    ...
    describe("First bid", () => {
      it("should set the first bid time", async () => {
        await ethers.provider.send("evm_setNextBlockTimestamp", [1658070015]);
        await auctionHouse.createBid(0, ONE_ETH.div(2), {
          value: ONE_ETH.div(2),
        });
    ...
    describe("Bid MaxPrice", () =>{
      ...
      it("should transfer the NFT to bidder who places maxPrice", async () => {
        ...
        expect(await nftMedia.ownerOf(0)).to.eq(await bidderB.getAddress());
      });
      it("should pay the curator their curatorFee percentage", async () => {
        ...
        const expectedCuratorFee = "100000000000000000";
      ...
      it("should pay the creator the remainder of the winning bid", async () => {
        ...
        const expectedProfit = "1900000000000000000";
    ...
    describe("Second bid", () => {
      ...
      it("should revert if the bid is smaller than the last bid + minBid", async () => {
        await expect(
          auctionHouse.connect(bidderB).createBid(0, ONE_ETH.div(2).add(1), {
            value: ONE_ETH.div(2).add(1),
          })
        ).to.be.rejectedWith(
          Error,
      ...
      it("should not update the firstBidTime", async () => {
        ...
        await auctionHouse.connect(bidderB).createBid(0, ONE_ETH, {
          value: ONE_ETH,
        });
  ...
  describe("should check startTime with bid creation time", () =>{
    let admin: SignerWithAddress;
    let curator: SignerWithAddress;
    let bidderA: SignerWithAddress;
    let bidderB: SignerWithAddress;
      ...
      it("should revert if the startTime is greater than bid creation time", async () => {
        await auctionHouse.connect(curator).setAuctionApproval(0, true);
        await ethers.provider.send("evm_setNextBlockTimestamp", [1658070215]);
        await expect(
          auctionHouse.connect(bidderA).createBid(0, ONE_ETH, { value: ONE_ETH })
        ).to.be.rejectedWith(Error, "auction has not yet started");
      });
    })
  ...
  describe("End the Auction", () => {
    ...
    it("should cancel the auction if the winning bidder is unable to receive NFTs", async () => {
      ...
      await badBidder.placeBid(0, ONE_ETH, { value: ONE_ETH });
      const endTime =
    ...
    describe("Final NFT Transfer to Winning Bidder", () => {
      ...
      it("should transfer the NFT to the winning bidder", async () => {
        ...
        expect(await nftMedia.ownerOf(0)).to.eq(await bidder.getAddress());
      });
